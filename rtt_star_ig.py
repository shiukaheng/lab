...
from inverse_geometry_utils import generate_cube_pos
...
class RTTStarImpl(RTTStar):
    ...
    def check_point_collision(self, point: np.ndarray) -> bool:
        q, success = computeqgrasppose(self.robot, self.q_init, self.cube, generate_cube_pos(*point), self.viz)
        return not success
    
    def check_edge_collision(self, start: np.ndarray, end: np.ndarray) -> (bool, Optional[np.ndarray]):
        return super().check_edge_collision(start, end)
